...
1. C的结构体内不允许函数存在，C++允许有内部成员函数，且允许该函数是虚函数
...
## [inline和define区别](https://blog.csdn.net/ENSHADOWER/article/details/86632685)
...
1. define只是简单文本替换，const是c++语言中关键字，其声明的常量具有类型，并在内存中分配存储空间
2. define不进行安全检查，const会进行
3. define定义的常量是全局的，整个文件中空间；const作用域受位置影响
4. define在预处理阶段执行，const在编译时创建常量

## 内存区介绍

1. 栈：保存局部变量和函数调用的上下文信息(函数参数、函数调用的返回地址)，后进先出，编译器自动管理
2. 堆：保存动态创建的对象和数据结构，手动分配和释放
3. 全局区/静态区：保存全局变量和静态变量；静态变量在程序生命周期内存在，只初始化一次；全局变量整个程序可见，但受限于定义它们的文件或代码块
4. 常量区：用于保存常量数据，存储在只读内存，且无法修改，整个生命周期都存在
5. 代码区：保存程序的二进制代码，存储在可读内存，无法修改。包含程序的指令集，用于执行程序的各种操作

## struct和union区别

1. struct每个成员都有自己的存储空间，union所有成员共享相同的存储空间，为最大成员的大小
2. struct可以同时访问结构的所有成员，union一次只能访问一个成员，当写入一个成员后，其余成员内容未定义
3. struct间几个相关变量组合到单一名字下，union用于节省内存。
4. struct可以同时初始化所有成员，union只能初始化第一个成员

## socket在本机和网络通信的区别

|            | 本机通信                 | 网络通信                                                     |
| ---------- | ------------------------ | ------------------------------------------------------------ |
| 通信范围   | 同一台机器的两个进程之间 | 两台不同计算机上的进程之间或同一台机器上的进程与其它机器上的进程之间 |
| 通信方式   | 不会真正传输到网络中     | 使用网络协议，通过网络设备在计算机之间传输                   |
| 地址方式   | 使用文件路径作为地址     | 使用IP地址和端口号来标识特定的socket                         |
| 延迟和带宽 | 较低延迟和较高带宽       | 受到网络、路由等影响                                         |
| 安全性     | 安全                     | 需要采取加密和其它安全措施                                   |
| 用途       | 系统内部的服务之间的也行 | 分布式系统中，如web客户端和web服务器                         |
...
[内存泄漏常见三种情况](https://blog.csdn.net/weixin_44718794/article/details/107071169)
...
p=np;//此时p被np指针重新赋值，但是p原来指向的内存无法被释放
```
...
* 减少线程切换开销
  * 避免频繁创建和销毁线程，使用线程池
  * 减少线程上下文切换，设置合理线程数量和避免不必要的线程挂起
* 优化线程任务分配
  * 确保线程平均分配工作量，避免某些线程过载而其它线程空闲
  * 使用任务队列和工作窃取算法来动态平衡不同线程的工作负载
* 避免阻塞和长时间等待
  * 减少IO操作，使用异步IO模型
  * 避免长时间的计算或等待外部资源
* 减少锁的竞争
...
* 开放封闭原则：通过扩展软件实体来解决需求变化，而不是通过修改已有代码
...
- **内存和资源**: 进程有独立的内存空间，线程共享内存空间。
- **通信**: 线程间的通信比进程间通信更容易和更有效。
- **依赖性**: 线程依赖于进程存在，而进程可以独立于其他进程存在。
- **创建和管理**: 创建和管理线程的开销小于进程。
- **可靠性**: 一个线程的崩溃可能影响同一进程中的其他线程，而一个进程的崩溃通常不会影响其他进程。

### 使用场景

- 使用多进程来实现强隔离、安全和稳定性更高的并发。
- 使用多线程来实现更高效的资源利用和并发操作，尤其是在计算密集型任务中。
...
* 资源一次性分配：一次性分配所有资源，就不会有请求了(破坏请求条件)
...
`EDA`是`Electronic Design Automation`，电子设计自动化，主要用于协助工程师设计、验证、模拟和制造各种电子系统、电路和芯片，提高设计效率、降低成本、减少设计错误并加速产品开发周期。EDA软件包含仿真工具、设计工具还有验证工具。

- **仿真**工具对所提出的电路进行描述，并在实现之前预测其行为。
- **设计**工具对所提出的电路功能进行描述，并组装一系列用于实现该功能的电路元件。这既是一个逻辑过程（组装和连接电路元件），也是一个物理过程（创建各种将在制造期间实施电路的互连几何形状）。所交付的这些工具结合了全自动化和交互式引导功能。
- **验证**工具检查芯片的逻辑或物理表示，以确定最终设计是否正确连接，并提供所需的性能。

当下先进的半导体器件包含数十亿个电路元件，如果不引入EDA管理这种级别的复杂程度，就不可能设计和制造当今的半导体器件。以及微小误差可能造成制造出来的芯片无法“修补”，必须重新设计和制造，耗时且成本高昂。

`fpga`是`Field Programmable Gate Array`，是一个可以通过编程来改变内部结构的芯片，编程后直接使用。相比单片机，效率更高、功耗更低，但是难开发成本高。

## 什么是autosar和aspice

* [`AUTOSAR`](https://zhuanlan.zhihu.com/p/118849539)：`Automotive Open System Architecture`，汽车开放系统架构
  * **由全球各家汽车制造商、零部件供应商以及各种研究、服务机构共同参与的一种汽车电子系统的合作开发框架，并建立了一个开放的汽车控制器（ECU）标准软件架构。**
  * **提升OEM(整车厂)以及供应商之间软件模块的可复用性和可互换性来改进对复杂汽车电子电气架构的管理。**
    * 对应用软件与底层软件之间以及应用软件之间的接口进行标准化
    * 给出一个控制器软件参考架构
    * 规范分布式开发流程中的交换格式
  * 
* [`ASPICE`](https://zhuanlan.zhihu.com/p/62139708)：`Automotive Software Process Improvement and Capacity Determination`，汽车软件过程改进及能力评定
  * 
