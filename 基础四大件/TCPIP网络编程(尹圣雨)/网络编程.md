...
```
### 第十七章 <span id="echo_epoll">基于epoll的回声服务器</span>
#### 服务端
```c++
#include<stdio.h>
#include<stdlib.h>
...
```
### <span id="echo_EPLTserv">第十七章 条件触发服务器</span>
#### 服务端
...
### <span id="echo_EPETserv">第十七章 边缘触发服务器</span>
#### 服务端
```c++
#include<stdio.h>
#include<stdlib.h>
...
### <span id="multithread">第十九章 多线程并发服务</span>
#### 服务端
```c++
#include<stdio.h>
#include<stdlib.h>
...
```
## 第十六章 关于I/O流分离的其它内容
<span id="dup&dup2">文件描述符的复制方法</span>
```c++
#include<unistd.h>
...
### 实现epoll时必要的函数和结构体
epoll服务器端实现要用到3个函数和1个结构体
* 3个函数
  * epoll_create：创建保存epoll文件描述符得空间
  * epoll_ctl：向空间注册并注销文件描述符
  * epoll_wait：与select函数类似，等待文件描述符发生变换
* 1个结构体
```c++
struct epoll_event
{
...
typedef union epoll_data//注意epoll_data是一个联合
{
    void* ptr;
    int fd;
    _unit32_t u32;
...
1. 在 epoll_ctl 中：指定向 epoll 例程注册文件描述符所关注的事件。用到的是 epoll_event 中的 events 成员。
2. 在 epoll_wait 中：保存发生事件的文件描述符集合。用到的是 epoll_event 中的 data 成员的 fd 成员。

### epoll_create
epoll 中由操作系统保存要监视的文件描述符，使用 epoll_create 函数向操作系统<strong style="color:red">申请创建保存文件描述符的空间</strong>。这对应使用 select 函数时声明 fd_set 变量来保存要监视的文件描述符。
```c++
#include <sys/epoll.h>
...
```
> Linux 2.6.8之后内核会完全忽略size参数，只需将size设为0就行。
> 查看Linux内核版本命令：`cat /proc/sys/kernel/osrelease`

### epoll_ctl
生成epoll例程后，调用该函数在其内部<strong style="color:red">注册监视对象文件描述符</strong>。
```c++
#include <sys/epoll.h>
...
### 条件触发和边缘触发
epoll的两种触发方式：
* 条件触发 LT(Level Trigger)，又叫电平触发
* 边缘触发 ET(Edge Trigger)

二者区别在于发生事件的时间点：
* 条件触发：只要有输入缓冲中有数据就一直通知该事件
* 边缘触发：只有当输入缓冲收到数据
...
#### errno
Linux套接字相关函数一般通过返回-1通知发生了错误，但是需要使用全局变量errno来提示产生错误的具体原因。
...
```
cmd可取值：
* F_GETFL：fcntl用于获取文件描述符filedes的属性(返回相应值)
* f_SETFL：更改文件描述符的属性

将套接字改为非阻塞的2条语句：
```c++
int flag=fcntl(fdmF_GETFL,0);//获取之前的属性
fcntl(fd,F_SETFL,flag|O_NONBLOCK);//在此基础上添加非阻塞状态
```
此后使用read对文件fd进行读取时都是非阻塞模式。
...
![](网络编程.assets/进程的内存结构.png)
![](网络编程.assets/线程的内存结构.png)

<strong style="color:red">多个线程会共享数据区和堆，每个线程维护自己的栈。</strong>
一种形式的进程和线程定义：
* 进程：在操作系统中构成单独执行流的单位。
* 线程：在进程中构成单独执行流的单位。
...
```
![](网络编程.assets/pthread_join函数使用.png)

### 可在临界区内调用的函数
线程安全函数被多个线程同时调用时也不会引发问题，非线程安全函数被同时调用时会引发问题。

可以通过使用`-D_REENTRANT`选项来将非线程安全函数调用为线程安全函数。
...
```
### 线程存在的问题和临界区
任何内存空间，只要被同时访问都可能发生问题
临界区指函数中同时运行多个线程时会引起问题的代码块，通常位于由线程运行的函数内部。
### 线程同步
线程同步用于解决线程访问顺序引发的问题，需要同步的情况可以从如下两方面考虑：
* 同时访问同一内存空间时发生的情况
* 需要指定访问同一内存空间的线程执行顺序问题

下面为两种常用的同步技术：互斥量和信号量。

#### 互斥量(Mutual Exclusion)
互斥量不允许多个线程同时访问，主要用于解决线程同步访问的问题。
...
如果忘了调用 pthread_mutex_unlock，其他想要进入临界区并调用了 pthread_mutex_lock 的线程会一直阻塞，进入<strong style="color:red">死锁状态</strong>。
...
#### 信号量
信号量与互斥量原理相似，但功能更多。
信号量除了可以解决同时访问内存的问题，还可以用来控制线程顺序。
**1.信号量的创建和销毁**
```c++
#include <semaphore.h>
...
// 功能：创建信号量
// 参数：sem：用于保存创建的信号量；
//pshared：取 0 时，创建只允许一个进程内部使用的信号量，取其他值时，创建可由多个进程共享的信号量；value：要创建的信号量的初始值；
// 返回值：成功时返回 0，失败时返回其他值。
...
<strong style="color:red">信号量的使用原理</strong>：当信号量为 0 时，sem_wait 函数会阻塞，直到其他线程调用 sem_post 函数使信号量值变为 1 为止。
```c++
#include <semaphore.h>
...
```
<strong style="color:red">使用方式</strong>：在进入临界区前使用sem_wait来将信号量减为0(假设信号量初始值为1)，临界区执行结束后用sem_post来将信号量加到1。

上面这种信号量的值在0和1之间交换的使用方式称为二进制信号量。
...
void* read(void *arg)
{
    for(int i=0;i<5;i++)
    {
        ...
        //刚开始sem_two=1,调用wait将其-1变为0
        //继续scanf输入，同时将sem_one+1
        sem_wait(&sem_two);
        scanf("%d",&num);
        sem_post(&sem_one);
    }
...
void* accu(void *arg)
{
    ...
    for(int i=0;i<5;i++)
    {
        //刚开始sem_one=0，需要等待read函数中post来讲sem_one+1才能继续
        sem_wait(&sem_one);
        sum+=num;
        //将sem_two+1
        sem_post(&sem_two);
...
### 线程的销毁方式
Linux不会自动销毁线程创建的内存空间，所以需要明确方法来销毁线程
销毁线程的两种方式：
1. 调用`pthread_join函数`。此函数不仅会等待指定的线程终止，还会引导线程的销毁。
2. 调用`pthread_detach函数`。此函数会将主线程与指定的子线程分离，分离后的子线程执行结束，资源会自动回收。

**pthread有joinable和unjoinable两种状态**
1. joinable状态：默认状态。当线程函数执行结束时或pthread_exit时不会释放线程所长堆栈和线程描述符等资源。只有当调用了pthread_join之后这些资源才会被释放。
2. unjoinable状态：线程占用的资源会在线程函数退出时或pthread_exit时自动释放。pthread_detach()函数就是分离线程，即将线程状态转换为unjoinable状态，以保证资源的释放。
> unjoinable状态可以在pthread_create时指定。
```c++
#include <pthread.h>
...
### 第24章 制作HTTP服务端
#### Web服务端
定义：基于HTTP协议，将网页对应文件传输给客户端的服务器端。

HTTP协议：Hypertext Transfer Protocol缩写，是以超文本传输为目的而设计的应用层协议。
...
unsigned WINAPI RequestHandler(void *arg)
{
	...
	if(strcmp(method, "GET"))    // 查看是否为HTTP方式的请求
		SendErrorMSG(hClntSock);
